...
## Epilogue: one more owned exception, also a failure

After I wrote this post, I gave the project one last owned exception. I combined the failed trend-following and mean-reversion strategies into a single regime-switching strategy that detects the market's mood (rising, sideways, or falling) and picks the matching tactic, and I tested it under a deliberately revised bar: I lowered the Sharpe pass mark from 1.0 to 0.85, an owned choice I document openly. It passed the SIA pre-screen for the first time in the project's history, and then lost money out-of-sample anyway. The full story, including why the regime-switching itself worked but the combination still failed, is in [the epilogue post](./MSS_POST.md). I am not done looking, and that post ends with an open invitation: if you work in this field and have a signal I have not tried, bring it, and I will run it through the same test with the bar back at 1.0.

## Footnotes / references

- **The case study**: [`./case-study/00-readme.md`](./case-study/00-readme.md) — reading guide for the eleven curated documents (the eleventh is the MSS epilogue, [`./case-study/11-mss-kill-adr-0027.md`](./case-study/11-mss-kill-adr-0027.md)). The audit corrigendum is at [`./case-study/10-audit-corrigendum.md`](./case-study/10-audit-corrigendum.md) as a self-contained teaching artefact about AI-assisted research failure modes.
